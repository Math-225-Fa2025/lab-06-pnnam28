...
```{r load-data-staff, message = FALSE, eval =TRUE, echo=TRUE}
...
If there are 5 faculty types and 11 years of data, how many rows will we have?
  ...
  Answer: The long format will have 55 rows Because there will be a modifier for each id. 
...
staff_long %>%
  ...
  labs(
    title = "Trends in Instructional Staff Composition (1987–2021)",
    x = "Year",
    y = "Proportion of Instructional Staff",
...
2.  Suppose the objective of this plot was to show that the proportion of part-time faculty have gone up over time compared to other instructional staff types.
  ...
  Answer: I would say this chart right now done a perfect job to compare the rising percentage of part time over year compare with others employment type. However, i think a bar plot also a good ideas to show the grow of part time job over the year with the x will be time, y is the percentage and facet of other employment type.
...
staff_long %>%
  ...
  labs(
    title = "Growth in Instructional Staff Composition Over Time",
    x = "Year",
...
4.  Can you help them make improve it? First, brainstorm how you would improve it. Then create the improved visualization and write up the changes/decisions you made as bullet points. It's ok if some of your improvements are aspirational, i.e. you don't know how to implement it, but you think it's a good idea.
 ...
 Answer: First, i would change the pie chart to vertical bar chart to show the difference between the difference Aquaculture and Capture. I think this is a better way to show the difference between multiple fields. For the plot, i would choose the top 10 countries so that it will be more concentrate and clearer dataset. 
 
 *Limited the plot to the top 10 countries by total fishery production to reduce clutter and make comparisons easier.

 *Replaced the 3D pie charts with a stacked bar chart, which allows more accurate comparison of Capture vs. Aquaculture.

 *Used horizontal bars (coord_flip()) so country names are readable.

 *Kept a single common scale for production so the relative size of countries is clear.

 *Used a simple, minimal theme and clear labels to improve readability and focus on the data.
...
```{r remodel_and_pivotdataset}
# 1. Create total production and keep only the top 10 countries
fisheries_top10 <- fisheries %>%
  mutate(total = capture + aquaculture) %>%
  slice_max(total, n = 10)

# 2. Pivot to long format just for those 10 (much cleaner)
fisheries_long <- fisheries_top10 %>%
  pivot_longer(
...
fisheries_long %>%
  ggplot(aes(x = reorder(country, total), y = value, fill = production_type)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Top 10 Countries by Fishery Production (2016)",
    x = "Country",
